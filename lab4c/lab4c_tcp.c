...
void write_shutdown() {
   ...
   if (print_temp)
    { 
       write(server_socket, time, 9);   
       write(server_socket, "SHUTDOWN\n", 9);
    }
   exit(0);
}
...
void process_input() {
  int check = read(server_socket,buffer,BUFF_SIZE);
  int i;
  if (check > 0)
    {
      for (i = 0; i < check; i++)
...
write(server_socket, str_id, strlen(str_id));
...
if (print_temp)
     {
       ...
       time_info = localtime(&curr_time);
        ...
        write(server_socket,temperature,strlen(temperature));
