...
void write_shutdown() {
   ...
   if (print_temp)
    { 
       SSL_write(sslClient, time, 9);   
       SSL_write(sslClient, "SHUTDOWN\n", 9);
    }
   exit(0);
}
...
void process_input() {
  int check = SSL_read(sslClient,buffer,BUFF_SIZE);
  int i;
  if (check > 0)
    {
      for (i = 0; i < check; i++)
...
if (SSL_set_fd(sslClient, server_socket) == 0)
    {
        fprintf(stderr, "Unable to set file descriptor with SSL\n");
        exit(2);
    }
...
struct pollfd Input;
Input.fd = server_socket; //Changed
Input.events = POLLIN;
...
SSL_write(sslClient, str_id, strlen(str_id));
...
if (print_temp)
     {
       ...
       time_info = localtime(&curr_time);
        ...
        sprintf(temperature,"%02d:%02d:%02d %.1f\n",time_info->tm_hour,time_info->tm_min,time_info->tm_sec,temp_conversion(thermo_value));
        SSL_write(sslClient,temperature,strlen(temperature));
